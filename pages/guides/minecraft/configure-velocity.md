...
После первого запуска Velocity создаст файл конфигурации `velocity.toml` — это главный конфиг твоего прокси.
...
Если прокси и игровые серверы расположены на одной ноде хостинга, можно оптимизировать соединение между ними.

Вместо внешнего IP (например, `123.123.123.123:25566`) в списке серверов Velocity укажи локальный адрес `127.0.0.1:порт`. Трафик не будет выходить во внешнюю сеть, а значит задержка (пинг) станет заметно ниже.
