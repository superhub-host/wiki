...
| **Java 22** | Экспериментальная поддержка для отдельных ядер 1.21.X |
| **Java 25** | Подойдёт для некоторых версий и ядер 1.21.X, а также 26.Х |

!> **Paper 1.16.5** официально поддерживает только до Java 16. Для запуска на Java 17 добавьте флаг `-DPaper.IgnoreJavaVersion=true` в аргументы JVM.
