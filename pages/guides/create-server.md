...
Самое главное, что тебе пригодится, — сам исполняемый файл сервера, его также называют «ядром». В [нашей статье](/guides/minecraft/change-version) есть удобная табличка со всеми популярными ядрами и ссылками на их скачивание. Для примера рассмотрим классический вариант — Paper. Конкретно он предоставляет некоторые оптимизации, позволяющие работать ему быстрее, чем обычный, «ванильный», сервер, а также позволяет устанавливать плагины, в том числе разработанные для Bukkit и Spigot.
